...
export default function StatusBadge({
  ...
  const options: PillDropdownOption<TaskStatus>[] = STATUS_OPTIONS.map(
    (value) => ({
      value,
      label: value,
      ...
      icon: (
        <span
          className={`h-2 w-2 rounded-full ${statusStyles[value].dot}`}
        />
      ),
    })
  );
    ...
    <div className='flex flex-wrap items-center gap-3'>
      ...
      <div className='min-w-[8rem]'>
        <PillDropdown
          ...
          onChange={(value) => {
            if (!value) return;
            void changeStatus(value as TaskStatus);
          }}
        />
      </div>
